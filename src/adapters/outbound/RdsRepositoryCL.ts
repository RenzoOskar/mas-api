...
export class RdsRepositoryCL implements IRdsRepository {
  private pool;
  ...
  async putItem(item:ICreateRequest): Promise<void> {
    try {
      const sql = `INSERT INTO appointments_cl (insuredId, scheduleId, countryISO) VALUES (?, ?, ?)`;
      let [result] = await this.pool.execute(sql, [item.insuredId, item.scheduleId, item.countryISO]);
      console.log("Insert result:", result);
      return (result as any).insertId;
    } catch (error) {
      console.error("Error inserting appointment in RDS (CL):", error instanceof Error ? error.message : error);
      console.error("Stack:", error instanceof Error ? error.stack : '');
      throw new Error("RDS CL insert failed");
    }    
  }
}